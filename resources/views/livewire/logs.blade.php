  ...
  <div>
    ...
    @elseif($showDetails)
      @if(count($data['missingEntries']) > 0)
        <div class="flex justify-center items-center mt-8">
          <div class="p-4 rounded-lg shadow-lg">
            <h1 class="text-xl font-bold mb-4">Missing dates</h1>
            <table class="w-full">
      ...
      @if(count($data['lesserHours']) > 0)
        <div class="flex justify-center items-center mt-8">
          <div class="p-4 rounded-lg shadow-lg">
            <h1 class="text-xl font-bold mb-4">Missing Hours</h1>
            <table class="w-full">
              <thead class="bg-blue-400 text-white">
              <tr>
                <th class="px-4 py-2">Date</th>
      ...
      @if(count($data['missingNotes']) > 0)
        <div class="flex justify-center items-center mt-8">
          <div class="p-4 rounded-lg shadow-lg">
            <h1 class="text-xl font-bold mb-4">Missing Notes</h1>
            <table class="w-full">
              <thead class="bg-blue-400 text-white">
              <tr>
                <th class="px-4 py-2">Date</th>
              ...
              @foreach ($data['missingNotes'] as $item)
                <tr>
                  <td class="px-4 py-2">{{ $item }}</td>
                  <td class="px-4 py-2">
                    <a href="/log-time/{{ $item }}" class="btn-add">Edit</a>
                  </td>
                </tr>
              @endforeach
              </tbody>
